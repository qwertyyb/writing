...
import BlockSelectorVue from './BlockSelector.vue'
...
class BlockSelector implements PluginView {
  vm: App
  vmProps = reactive({
  ...
  constructor(editorView: EditorView) {
    ...
    const vm = createApp({
      render: () => h(BlockSelectorVue, this.vmProps as any)
    })
...
export const blockSelector = () => {
  return new Plugin({
    view(editorView) {
      const blockTool = new BlockSelector(editorView)
      return blockTool
    },
  })
}
